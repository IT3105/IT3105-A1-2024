// You could separate these class declarations to a separate js, for example MyClass.js and use the export functionality
// for example 'export class MyClass {}' and then import it in the main.js using the 'import' functionality for example
...
class BudgetTracker {
  ...
  loadTransactions() {
    ...
    if (storedData) {
      ...
      this.transactions = data.transactions.map(
        (t) =>
          new Transaction(t.id, t.date, t.description, t.amount, t.category)
      );
  ...
  addTransaction(description, amount, category) {
    ...
    this.expensesByCategory[category] =
      (this.expensesByCategory[category] || 0) + transaction.amount;
  ...
  updateIncomeAndExpenses() {
    ...
    this.expensesTotal = Object.values(this.expensesByCategory).reduce(
  ...
  updateExpenses() {
    ...
    this.transactions.forEach((t) => {
      this.expensesByCategory[t.category] =
        (this.expensesByCategory[t.category] || 0) + t.amount;
    });
  }
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    if (description && !isNaN(amount) && category) {
      const transaction = budgetTracker.addTransaction(
        description,
        amount,
        category
      );
...
function updateTransactionTable() {
  ...
  budgetTracker.transactions.forEach((transaction) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <button class="text-blue-600 hover:text-blue-800 mr-2" onclick="openEditTransactionModal(${
                  transaction.id
                })">Edit</button>
                <button class="text-red-600 hover:text-red-800" onclick="deleteTransaction(${
                  transaction.id
                })">Delete</button>
            </td>
        `;
...
function openEditTransactionModal(id) {
  ...
  if (transaction) {
    ...
    document.getElementById("modalEditAmount").value =
...
document.getElementById("saveEditTransaction").addEventListener("click", () => {
  ...
  const description = document
    .getElementById("modalEditDescription")
    .value.trim();
  ...
  if (id && (description || amount || category)) {
    const updatedTransaction = budgetTracker.updateTransaction(
      parseInt(id),
      description,
      amount,
      category
